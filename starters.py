...
def starters(root,root1,root2,type):
        a=StringVar()
        result = c.execute("select recipe_name from "+type+" where category='Starters'")
        result = c.fetchall()
        #print(result)
        #print(result)
        m=Toplevel(root)
        ...
        i=0
        for item in result:
                r=Radiobutton(m,variable=a,value=item[0],bg="burlywood2")
                b11=Button(m, bg="MAROON",fg="burlywood2", text=item[0] ,font="Bold 15",pady=0.5,relief = FLAT,height=1,width=30)
                li.append(b11)
                b11.place(x=500,y=j)
                r.place(x=450,y=j)
                j+=150
        ...
        b4=Button(m, bg="MAROON", fg="burlywood2", text='SHOW RECIPE',font="Bold 15",pady=0.5,relief = FLAT,height=1,width=30,command=lambda : display(m,a.get(),type))
